...
def apply_r1_distilled_template(question: str):
    return (
        "<｜begin▁of▁sentence｜>Please reason step by step, and put your final answer within \\boxed{}.<｜User｜>" + question + "<｜Assistant｜><think>\n"
    )

def apply_r1_distilled_modified_template(question: str):
    return (
...
def main(
    ...
    tasks: list = ["aime", "amc23", "math", "minerva", "olympiad_bench"],
    ...
    elif template == "r1-distilled":
        math_reward_fn = boxed_reward_fn
        apply_template = apply_r1_distilled_template
    elif template == "r1-distilled-modified":
        math_reward_fn = boxed_reward_fn
        apply_template = apply_r1_distilled_modified_template
    else:
        raise ValueError
    ...
    results_to_save["avg"] = float(avg_score)
    
    # Create folder if it doesn't exist
    save_folder = os.path.join(model_name, "eval_deepseek_longcontext")
    os.makedirs(save_folder, exist_ok=True)
    
    # Generate filename with datetime precise to seconds
    ...
    if save:
        ...
        fn = f"save_eval/{fn}_template_{template}_temp{temperature}_topp{top_p}_n{n_samples}.json"
